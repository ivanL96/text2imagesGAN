...
class BBOX_NET(nn.Module):
    ...
    def forward(self, labels, transf_matr_inv, max_objects):
        ...
        label_layout = torch.FloatTensor(labels.shape[0], self.c_dim, 16, 16).fill_(0)
...
class STAGE1_G(nn.Module):
    ...
    def forward(self, text_embedding, noise, transf_matrices_inv, label_one_hot, max_objects=3):
        ...
        local_labels = torch.FloatTensor(noise.shape[0], max_objects, self.ef_dim).fill_(0)
        ...
        h_code_locals = torch.FloatTensor(noise.shape[0], self.gf_dim // 4, 16, 16).fill_(0)
        for idx in range(max_objects):
...
class STAGE1_D(nn.Module):
    ...
    def _encode_img(self, image, label, transf_matrices, transf_matrices_inv, max_objects):
        ...
        h_code_locals = torch.FloatTensor(image.shape[0], self.df_dim * 2, 16, 16).fill_(0)
...
class STAGE2_G(nn.Module):
    ...
    def forward(self, text_embedding, noise, transf_matrices_inv,
        ...
        if cfg.USE_BBOX_LAYOUT:
            ...
            labels_layout = torch.FloatTensor(noise.shape[0], self.ef_dim, 16, 16).fill_(0)
        ...
        h_code_locals = torch.FloatTensor(h_code.shape[0], self.gf_dim, 64, 64).fill_(0)
...
class STAGE2_D(nn.Module):
    ...
    def _encode_img(self, image, label, transf_matrices, transf_matrices_inv, max_objects):
        ...
        h_code_locals = torch.FloatTensor(image.shape[0], self.df_dim * 2, 32, 32).fill_(0)
